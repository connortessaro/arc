...
if [[ $# -eq 0 ]]; then
  echo "Usage: scripts/arc-self-drive/run-code-via-ssh-tunnel.sh <code-subcommand...>" >&2
  exit 1
fi
...
if ! command -v curl >/dev/null 2>&1; then
  echo "curl is required to wait for the Arc remote tunnel." >&2
  exit 1
fi

SSH_ARGS=(ssh)
if [[ -n "$REMOTE_SSH_IDENTITY" ]]; then
  SSH_ARGS+=(-i "$REMOTE_SSH_IDENTITY")
fi

REMOTE_TOKEN="$(
  "${SSH_ARGS[@]}" "$SSH_TARGET" "python3 - <<'PY'
...
"${SSH_ARGS[@]}" -N -L "${LOCAL_TUNNEL_PORT}:127.0.0.1:${REMOTE_GATEWAY_PORT}" "$SSH_TARGET" >/dev/null 2>&1 &
TUNNEL_PID="$!"
...
OPENCLAW_CONFIG_PATH="${TEMP_CONFIG_DIR}/openclaw.json" \
  ARC_REMOTE_SSH_TARGET="$SSH_TARGET" \
  ARC_REMOTE_SSH_IDENTITY="$REMOTE_SSH_IDENTITY" \
  ARC_REMOTE_REPO_ROOT="$REMOTE_REPO_ROOT" \
  ARC_REMOTE_GATEWAY_PORT="$REMOTE_GATEWAY_PORT" \
  ARC_REMOTE_TUNNEL_PORT="$LOCAL_TUNNEL_PORT" \
